...
// ----------------------------------------------------------------------------
// 按实例串行化: 同一 ECS 实例上的远程执行共用 Workbench 会话输出流, 并发调用
// 会互相串流(后台任务 stdout 混入前台体检输出、体检段落错位)。CLI 没有"每次
// 调用独立会话"的开关(session 只有 list/close), 因此在插件侧对同一 instance_id
// 的操作加 FIFO 互斥锁: 同实例串行, 不同实例仍可并行。
// 所有触达实例的 CLI 操作(exec/diagnose/deploy/upload/download + 设置页 RPC)
// 都应经过这里。
// ----------------------------------------------------------------------------

...
// ----------------------------------------------------------------------------
// 并发闸门(S7): 按 limit 并发执行 worker, 结果按输入顺序返回。
// 同实例的互斥由 withInstanceLock 保证, 因此跨实例并发天然安全。
// ----------------------------------------------------------------------------
...
export async function removeLocalFile(ctx, filePath, signal) {
  const subprocess = ctx.get('subprocess')
  ...
  for (const tool of LOCAL_REMOVE_TOOLS) {
    ...
    if (exe === undefined) continue
    try {
      const r = await runProcess(ctx, [exe, ...tool.args(filePath)], signal, { stdoutMaxBytes: 16 * 1024 })

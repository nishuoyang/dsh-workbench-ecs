...
import { listRunbookNames, loadRunbook, RUNBOOK_DIR } from './runbooks.js'
...
export function apply(ctx) {
  ...
  for (const factory of TOOL_FACTORIES) {
    ctx.tools.register(defineTool(toLossless(factory(ctx))))
  }
  ...
  // 会话工作区根目录: runbook 文件按 <workspaceRoot>/.dsh/workbench-ecs/runbooks/
  // 解析; 与 ecs_deploy 工具用的是同一个来源(sandboxPolicy.workspaceRoot)。
  const sandboxPolicy = ctx.get('sandboxPolicy')
  const workspaceRoot = sandboxPolicy !== undefined && sandboxPolicy !== null &&
    sandboxPolicy.workspaceRoot !== undefined && sandboxPolicy.workspaceRoot !== null
    ? String(sandboxPolicy.workspaceRoot) : undefined

  const core = createSettingsCore(runCli, {
    runbookDir: (workspaceRoot !== undefined ? workspaceRoot.replace(/[\\/]+$/, '') + '/' : '') + RUNBOOK_DIR,
    listRunbooks: async () => await listRunbookNames(ctx, { workspaceRoot }),
    loadRunbook: async (name) => await loadRunbook(ctx, name, { workspaceRoot }),
    ...
    handler: async (req, res) => {
      try {
        ...
        switch (op) {
          ...
          case 'runbook-list': return sendJson(res, 200, await core.runbookList())

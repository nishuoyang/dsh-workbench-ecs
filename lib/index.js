// ============================================================================
// dsh-workbench-ecs —— 阿里云 Workbench CLI 包装插件 (DeepSeek Harness / Cordis)
// ----------------------------------------------------------------------------
// 入口: 标准 Cordis 插件形态, 提供两部分能力:
//   A. 模型可见工具 (agent 会话):
//      ecs_list       列出指定地域的 ECS 实例
//      ecs_exec       在实例上执行远程命令/脚本(守卫/只读护栏/批量/后台/detach)
//      ecs_log        按字节游标续读远端文件(长任务日志)
//      ecs_upload     上传本地文件到实例(OSS 中继, ≤1GB, 可选 sha256 校验)
//      ecs_download   从实例下载文件到本地
//      ecs_diagnose   一键只读体检(主机/负载/内存/磁盘/服务/进程/端口)
//      ecs_deploy     受控发布(上传 + sha256 校验 + 重启 + 健康检查 / steps 编排 / runbook)
//      ecs_runbook    工作区跑书的只读清点与静态校验(list/validate/plan, 不触达实例)
//      ecs_session    会话管理(list/close)
//   B. 设置页同源 RPC (web profile):
//      GET  /dsh-workbench-ecs/health         健康检查
//      POST /dsh-workbench-ecs/rpc            设置页 JSON RPC
//        op: status(20s缓存/force重查) | list | exec(守卫) | deploy |
//            runbook-list | runbook-validate(静态校验) | runbook-plan(预演) |
//            runbook-run(执行) | session-list | session-close
//
// 前置要求: 本机安装并配置阿里云 Workbench CLI(见 README):
//   Windows: irm https://workbench-cli.oss-cn-hangzhou.aliyuncs.com/install.ps1 | iex
//   Linux/macOS: curl -fsSL https://workbench-cli.oss-cn-hangzhou.aliyuncs.com/install.sh | bash
//   凭据: ~/.workbench/config.json(支持 AK/StsToken/RamRoleArn/CredentialsCmd/CredentialsURI)
// ============================================================================
import { defineTool } from '@deepseek-ai/dsh-tools'
import {
  runWorkbench, PLUGIN_VERSION, omitUndefined, localSha256, remoteSha256, delay, hasLocalTimer,
  resolveWorkspaceRoot,
} from './common.js'
import { createSettingsCore } from './settings-api.js'
import { listRunbookNames, loadRunbook, runbookDirOf, RUNBOOK_DIR } from './runbooks.js'
import { ecsListDefinition } from './tools/ecs-list.js'
import { ecsExecDefinition } from './tools/ecs-exec.js'
import { ecsLogDefinition } from './tools/ecs-log.js'
import { ecsUploadDefinition } from './tools/ecs-upload.js'
import { ecsDownloadDefinition } from './tools/ecs-download.js'
import { ecsDiagnoseDefinition } from './tools/ecs-diagnose.js'
import { ecsDeployDefinition } from './tools/ecs-deploy.js'
import { ecsRunbookDefinition } from './tools/ecs-runbook.js'
import { ecsSessionDefinition } from './tools/ecs-session.js'

export const name = 'dsh-workbench-ecs'

// 注入三项宿主服务:
//   tools       —— agent 会话的工具注册表 (A 部分)
//   webServer   —— web profile 的同源 HTTP 路由注册表 (B 部分)
//   subprocess  —— 本机进程执行服务; 设置页 RPC 与工具共用
// 面向部署: web profile 与 agent 会话均提供这三项服务。
export const inject = ['tools', 'webServer', 'subprocess']

// 工具工厂表: 每次 apply 生成定义并注册(注册随 fiber 自动清理)
const TOOL_FACTORIES = [
  ecsListDefinition,
  ecsExecDefinition,
  ecsLogDefinition,
  ecsUploadDefinition,
  ecsDownloadDefinition,
  ecsDiagnoseDefinition,
  ecsDeployDefinition,
  ecsRunbookDefinition,
  ecsSessionDefinition,
]

// DSH 工具管线对 execute 返回值与各展示投影做 lossless JSON 校验(isJsonValue):
// 任何 undefined 值属性 / NaN / -0 都会使整次调用失败(如 ecs_exec 报
// "value is not lossless JSON")。在注册边界统一剔除 undefined 值属性,
// 保证单个工具遗漏时也不会再次打断整次调用; 可表达内容原样保留。
// 结构: execute / presentCall / presentResult 在顶层, render / presentationMeta 在 output 下。
const OUTPUT_PROJECTOR_KEYS = ['render', 'presentationMeta']
const TOP_PROJECTOR_KEYS = ['presentCall', 'presentResult']
function wrapProjector(fn) {
  return (...fnArgs) => {
    const r = fn(...fnArgs)
    return r === undefined ? r : omitUndefined(r)
  }
}
function toLossless(def) {
  const wrapped = { ...def }
  if (typeof def.execute === 'function') {
    wrapped.execute = async (args, exec) => omitUndefined(await def.execute(args, exec))
  }
  if (def.output !== undefined && typeof def.output === 'object') {
    const out = { ...def.output }
    for (const key of OUTPUT_PROJECTOR_KEYS) {
      if (typeof out[key] === 'function') out[key] = wrapProjector(out[key])
    }
    wrapped.output = out
  }
  for (const key of TOP_PROJECTOR_KEYS) {
    if (typeof def[key] === 'function') wrapped[key] = wrapProjector(def[key])
  }
  return wrapped
}

export function apply(ctx) {
  // 最近一次 Agent 会话的工作区(v0.6.4 / D11): 设置页 RPC 没有会话上下文,
  // 但跑书目录必须与 Agent 看到的一致 —— 于是记住工具调用里解析出的会话工作区,
  // 面板默认跟随它(仍可用 dir 参数显式覆盖)。
  let lastSessionRoot

  function withSessionRoot(def) {
    const inner = def !== null && typeof def === 'object' ? def.execute : undefined
    if (typeof inner !== 'function') return def
    return Object.assign({}, def, {
      execute: async (args, exec) => {
        try {
          const root = resolveWorkspaceRoot(ctx, exec)
          if (root !== undefined) lastSessionRoot = root
        } catch (err) {
          /* 解析失败不影响工具本身 */
        }
        return await inner(args, exec)
      },
    })
  }

  // ---- A. 注册模型可见工具 ----
  for (const factory of TOOL_FACTORIES) {
    ctx.tools.register(defineTool(toLossless(withSessionRoot(factory(ctx)))))
  }

  // ---- B. 设置页同源 RPC 路由 ----
  const webServer = ctx.webServer
  if (webServer === undefined || typeof webServer.register !== 'function') return

  // runCli 适配层: common.runWorkbench 需要 ctx(subprocess/sandboxPolicy);
  // 设置页 RPC 只关心退出码与输出文本(runCli), runbook 编排则要完整的
  // 结果对象(runCliRaw: 含 stdout 截断/spill 信息), 两者同一实现。
  const runCliRaw = async (argv) => await runWorkbench(ctx, argv, undefined)
  const runCli = async (argv) => {
    const r = await runCliRaw(argv)
    return { exitCode: r.exitCode, stdout: r.stdout, stderr: r.stderr }
  }

  // 设置页的跑书目录: 显式 args.dir > 最近会话工作区 > 部署兜底。
  // (会话工作区来源见 common.resolveWorkspaceRoot: exec.agent.session.header.cwd)
  function runbookDirFor(args) {
    const raw = args !== null && typeof args === 'object' && args.dir !== undefined && args.dir !== null
      ? String(args.dir).trim() : ''
    if (raw.length > 0) return raw.replace(/[\\/]+$/, '')
    const root = lastSessionRoot !== undefined ? lastSessionRoot : resolveWorkspaceRoot(ctx, undefined)
    return runbookDirOf({ workspaceRoot: root })
  }

  const core = createSettingsCore(runCli, {
    runbookDirOf: (args) => runbookDirFor(args),
    listRunbooks: async (args) => await listRunbookNames(ctx, { runbookDir: runbookDirFor(args) }),
    loadRunbook: async (name, args) => await loadRunbook(ctx, name, { runbookDir: runbookDirFor(args) }),
    // 面板执行 runbook 时的本机执行适配器(与 ecs_deploy 的 stepAdapter 同形):
    // 差别只在"整段编排的实例锁由 settings 核心持", 因此这里同样用 locked: true
    // 拿远端 sha256, 不再二次取锁(不可重入)。
    makeStepsAdapter: (args) => ({
      run: async (argv) => await runCliRaw(argv),
      cwd: runbookDirFor({}),
      localSha256: async (file) => await localSha256(ctx, file, undefined),
      remoteSha256: async (remotePath, opts = {}) => await remoteSha256(ctx, args.instance_id, remotePath,
        { region: args.region, timeout: opts.timeout, locked: true }),
      sleep: (ms) => delay(ctx, ms),
      hasTimer: hasLocalTimer(ctx),
    }),
  })

  // 请求体上限: 防御面板误发巨型载荷 (正常 RPC 请求仅 KB 级)
  const MAX_BODY = 512 * 1024

  function sendJson(res, code, obj) {
    try {
      res.writeHead(code, {
        'Content-Type': 'application/json; charset=utf-8',
        'Cache-Control': 'no-store',
      })
      res.end(JSON.stringify(obj))
    } catch {
      /* 连接已断开 */
    }
  }

  function readBody(req) {
    return new Promise((resolveBody) => {
      const chunks = []
      let size = 0
      req.on('data', (chunk) => {
        size += chunk.length
        if (size > MAX_BODY) {
          req.destroy()
          resolveBody(null)
          return
        }
        chunks.push(chunk)
      })
      req.on('end', () => resolveBody(Buffer.concat(chunks).toString('utf8')))
      req.on('error', () => resolveBody(null))
    })
  }

  ctx.effect(() => webServer.register({
    kind: 'prefix',
    path: '/dsh-workbench-ecs',
    handler: async (req, res) => {
      try {
        const url = new URL(req.url, 'http://localhost')
        const p = url.pathname
        if (p === '/dsh-workbench-ecs/health') {
          return sendJson(res, 200, { ok: true, plugin: 'dsh-workbench-ecs', version: PLUGIN_VERSION })
        }
        if (p !== '/dsh-workbench-ecs/rpc') {
          return sendJson(res, 404, { ok: false, error: '未知路由: ' + p })
        }
        if (req.method !== 'POST') {
          return sendJson(res, 405, { ok: false, error: 'rpc 仅接受 POST' })
        }
        const raw = await readBody(req)
        if (raw === null) return sendJson(res, 400, { ok: false, error: '请求体过大或已断开' })
        let payload
        try {
          payload = JSON.parse(raw)
        } catch (e) {
          return sendJson(res, 400, { ok: false, error: '请求体不是合法 JSON' })
        }
        const op = payload != null ? payload.op : undefined
        const args = payload != null && payload.args !== undefined ? payload.args : {}
        switch (op) {
          case 'status': return sendJson(res, 200, await core.status(args))
          case 'list': return sendJson(res, 200, await core.list(args))
          case 'exec': return sendJson(res, 200, await core.exec(args))
          case 'deploy': return sendJson(res, 200, await core.deploy(args))
          case 'runbook-list': return sendJson(res, 200, await core.runbookList(args))
          case 'runbook-validate': return sendJson(res, 200, await core.runbookValidate(args))
          case 'runbook-plan': return sendJson(res, 200, await core.runbookPlan(args))
          case 'runbook-run': return sendJson(res, 200, await core.runbookRun(args))
          case 'session-list': return sendJson(res, 200, await core.sessionList())
          case 'session-close': return sendJson(res, 200, await core.sessionClose(args))
          default: {
            // 单个操作失败时返回 JSON 错误而非裸 500, 面板可直接展示
            const safe = String(op != null ? op : '(missing)').slice(0, 80)
            return sendJson(res, 400, { ok: false, error: '未知操作: ' + safe })
          }
        }
      } catch (err) {
        return sendJson(res, 500, {
          ok: false,
          error: String((err != null && err.message != null) ? err.message : err),
        })
      }
    },
  }))
}

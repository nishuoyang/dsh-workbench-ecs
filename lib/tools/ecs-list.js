...
export function ecsListDefinition(ctx) {
  ...
  function paginationOf(data, returned, limit) {
    ...
    if (nextToken === undefined && returned >= limit) {
      note = '返回条数已达 limit(' + limit + '), 可能存在下一页, 但 CLI 的 JSON 输出未包含 NextToken; ' +
        '可收紧过滤条件(instance_name/tag/status/vpc_id)缩小范围, 或为 CLI 提交 NextToken 透出需求'
    }
  ...
  return {
    ...
    async execute(args, exec) {
      ...
      if (args.vpc_id !== undefined) argv.push('--vpc-id', args.vpc_id)
      if (args.vswitch_id !== undefined) argv.push('--vswitch-id', args.vswitch_id)
      if (args.zone_id !== undefined) argv.push('--zone-id', args.zone_id)
      if (args.private_ip !== undefined && args.private_ip.length > 0) {
        argv.push('--private-ip', args.private_ip.map((ip) => String(ip)).join(','))
      }
      if (args.image_id !== undefined) argv.push('--image-id', args.image_id)
      const limit = Number.isFinite(Number(args.limit)) && Number(args.limit) >= 10
        ? Math.min(Math.floor(Number(args.limit)), 100) : 50
      if (args.limit !== undefined) argv.push('--limit', String(limit))
      if (args.next_token !== undefined && String(args.next_token).length > 0) {
        argv.push('--next-token', String(args.next_token))
      }

...
export function ecsUploadDefinition(ctx) {
  ...
  async function uploadDirectory(args, exec) {
    ...
    // 1) 本机归档(在实例锁之外, 纯本地操作)
    const tarRun = await runProcess(ctx, buildTarCreateArgv(tarExe, archiveLocal, args.local_dir), exec.signal, {
    ...
    let sha256Local
    if (verify) sha256Local = await localSha256(ctx, archiveLocal, exec.signal)
    ...
    try {
      const out = await withInstanceLock(args.instance_id, async () => {
        const up = await runWorkbench(ctx, uploadArgv, exec.signal)
        ...
        const ex = await runWorkbench(ctx, extractArgv, exec.signal)
        if (exec.signal.aborted) throw new Error('工具调用已被取消')
      ...
      // 3) 本地归档清理(尽力; 不影响结果)
      const removed = await removeLocalFile(ctx, archiveLocal, undefined)
      cleanup = removed
      ...
      await removeLocalFile(ctx, archiveLocal, undefined).catch(() => {})
      throw err
    }
  }

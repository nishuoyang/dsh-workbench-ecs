      ...
      render: (args, value) => {
        ...
        if (issues.length > 0) {
          ...
          for (const issue of issues) {
            // 结构类问题的文案本身已带 steps[i] 定位, 不要重复前缀
            const alreadyLocated = issue.step === undefined || String(issue.message).indexOf('steps[') === 0
            lines.push('  ' + (issue.level === 'error' ? '✘ [错误] ' : '⚠ [提醒] ') +
              (alreadyLocated ? '' : 'steps[' + issue.step + '] ') + issue.message)
          }
        }

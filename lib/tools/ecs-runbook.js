...
// ============================================================================
import { omitUndefined } from '../common.js'
import {
  RUNBOOK_DIR, listRunbookNames, loadRunbook, lintRunbook, parseRunbook, buildRunbookRun, summarizeRunbook,
} from '../runbooks.js'
...
export function ecsRunbookDefinition(ctx) {
  ...
  async function listRunbooks(exec) {
    ...
    const dir = (workspaceRoot !== undefined ? workspaceRoot.replace(/[\\/]+$/, '') + '/' : '') + RUNBOOK_DIR
    const entries = []

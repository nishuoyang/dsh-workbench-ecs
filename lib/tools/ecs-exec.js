...
export function ecsExecDefinition(ctx) {
      ...
      run: () => {
        ...
        const done = (async () => {
          while (!cancelled) {
            ...
            try {
              ...
              r = await withInstanceLock(id, () => runWorkbench(ctx, argv, undefined, { stdoutSpillMaxBytes: 8 * 1024 * 1024 }))
            } catch (err) {
              failures += 1
              if (failures > 5) {
        ...
        return {
          cancel: () => {
            ...
            withInstanceLock(id, () => runWorkbench(ctx, argv, undefined, { stdoutMaxBytes: 64 * 1024 })).catch(() => {})
          },
          done,
          readOutput: () => {
            const parts = []
  ...
  return {
    ...
    description: '通过本机阿里云 Workbench CLI 在指定 ECS 实例上执行远程命令(非交互), ' +
      ...
      '支持 instance_ids 批量执行(串行, 上限 ' + BATCH_LIMIT + ' 台)、run_in_background 后台任务, ' +

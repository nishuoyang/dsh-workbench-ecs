...
import {
  ...
  guardDestructiveCommand, omitUndefined, withInstanceLock,
} from '../common.js'
...
export function ecsExecDefinition(ctx) {
  ...
  return {
    ...
    async execute(args, exec) {
      ...
      if (args.run_in_background === true) {
        ...
        const jobId = jobs.start({
          ...
          run: () => {
            ...
            let cancelled = false
            const done = withInstanceLock(targets[0], async () => {
              if (cancelled) return { status: 'killed', detail: 'cancelled before start' }
              const h = await spawnWorkbench(ctx, buildArgv(targets[0]), undefined)
              handle = h
              const outcome = await h.done
              const code = outcome.exitCode != null ? outcome.exitCode : 0
      ...
      // ---- 单实例前台 ----
      if (targets.length === 1) {
        const r = await withInstanceLock(targets[0], () => runWorkbench(ctx, buildArgv(targets[0]), exec.signal, {
          stdoutSpillMaxBytes: 64 * 1024 * 1024,
        }))
        if (exec.signal.aborted) throw new Error('工具调用已被取消')
        return omitUndefined({ kind: 'single', command: args.command, command_line: commandLine(buildArgv(targets[0])), ...execOne(r, targets[0]) })
      }
      ...
      for (const id of targets) {
        try {
          const r = await withInstanceLock(id, () => runWorkbench(ctx, buildArgv(id), exec.signal, {
            stdoutSpillMaxBytes: 64 * 1024 * 1024,
          }))
          if (exec.signal.aborted) throw new Error('工具调用已被取消')

...
// ============================================================================
import {
  runWorkbench, commandLine, guardDestructiveCommand, guardReadOnly,
...
export function ecsDeployDefinition(ctx) {
  ...
  // --------------------------------------------------------------------------
  function stepAdapter(signal, args = {}) {
    return {
      run: async (argv) => {
      ...
      localSha256: async (file) => await localSha256(ctx, file, signal),
      ...
      remoteSha256: async (remotePath, opts = {}) => await remoteSha256(ctx, args.instance_id, remotePath,
        { region: args.region, signal, timeout: opts.timeout, locked: true }),
  ...
  async function runStage(argv, signal, opts = {}) {
    return await execStage(stepAdapter(signal), argv, opts)
  }
  ...
  async function executeSteps(args, exec, steps, extra = {}) {
    ...
    const adapter = stepAdapter(exec.signal, args)

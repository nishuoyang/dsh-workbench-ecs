...
import { parseRunbook, buildRunbookRun, summarizeRunbook } from './runbooks.js'
...
export function createSettingsCore(runCli, deps = {}) {
  ...
  async function resolveRunbookInput(raw, args) {
    ...
    if (typeof raw === 'string' && raw.length > 0) {
      const loaded = await deps.loadRunbook(raw)
    ...
    throw new Error('runbook 必须是名字字符串(读工作区 ' + String(deps.runbookDir) + '/<name>.json)或内联对象 { params?, steps }')
  }
  ...
  async function runbookList() {
    if (typeof deps.listRunbooks !== 'function') {
      return { ok: true, dir: String(deps.runbookDir != null ? deps.runbookDir : ''), runbooks: [] }
    }
    let names = []
    try {
      names = await deps.listRunbooks()
    } catch (err) {
      return { ok: false, error: message(err) }
    }
    const runbooks = []
    for (const name of names) {
      try {
        const loaded = await deps.loadRunbook(name)
        ...
        runbooks.push({ name, valid: false, error: message(err) })
      }
    }
    return { ok: true, dir: String(deps.runbookDir != null ? deps.runbookDir : ''), runbooks }
  }
  ...
  return { status, list, exec, deploy, sessionList, sessionClose, runbookList, runbookPlan, runbookRun }
}

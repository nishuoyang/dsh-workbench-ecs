...
window.__ModuleLoader__.load({
  ...
  factory: function (require) {
    ...
    function WorkbenchPanel() {
      ...
      const [rb, setRb] = React.useState({ list: [], dir: '', busy: false, err: '', target: '', params: '', active: '', preview: null, result: null, runBusy: false, error: '' })
      ...
      async function loadRunbooks() {
        ...
        try {
          const res = await call('runbook-list', {})
      ...
      const rbTable = rb.list.length === 0
        ...
        : el('table', { className: 'wbecs-table' },
          el('thead', null, el('tr', null, ['名称', '说明', '步骤', '类型', '参数占位', '操作'].map((hd) => el('th', { key: hd }, hd)))),
          el('tbody', null, rb.list.map((item) => el('tr', { key: item.name },
            ...
            el('td', null, el('div', { className: 'wbecs-actions' },
              ...
              el('button', { className: 'wbecs-btn wbecs-btn-primary', onClick: () => runRunbook(item.name, false), disabled: rb.runBusy || item.valid !== true }, '执行')))))))
      rows.push(el('div', { key: 'runbooks', className: 'wbecs-card' },
        ...
        el('div', { className: 'wbecs-row' },
          ...
          el('span', { className: 'wbecs-empty', style: { fontSize: 11 } }, rb.dir || '.dsh/workbench-ecs/runbooks'),
          el('input', {
        ...
        el('div', { className: 'wbecs-empty', style: { fontSize: 11, marginTop: 8 } },
          '首行为“预演”: 只回显将要下发的命令行, 不触碰实例; “执行”走与 Agent 侧 ecs_deploy 相同的编排引擎。破坏性命令面板直接拒绝(需用 Agent 工具经审批执行)。')))

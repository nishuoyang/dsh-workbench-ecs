...
> 日期: 2026-09-12
>
> **实施状态(2026-09-12 更新)**: **v0.4.0 已落地**并推上生产实例验证 ——
> `npm test`(13 项单元 + 冒烟/动态 body 一致性)全绿;`npm run test:e2e` 对
> `i-uf66ct2o35p7fjcd0sru` **20/20 通过**(含 F1 原始场景: 容器内 `node -e` 零转义直通)。
...
| **v0.5.0** | 长任务与会话 | S2 detach + `ecs_log` 游标 / S3 伪会话 / S7 并行批量+后台批量+JSON / S5b 目录递归 / 解 **D2、D3** | 长任务不再是轮询体力活;同实例并发不再互相拖死 |
...
7. **D2**:一个 10 分钟 detach 任务运行期间,同实例 `ecs_diagnose` 能正常返回(不再排队超时)—— v0.5.0;
8. **D3/S2**:detach 任务 3 次轮询拿到完整日志、**无重复段、无缺失段**;50MB 日志按 `after` 游标从头翻完 —— v0.5.0;
9. **S3**:同 `session_id` 继承 `cd`;不同 `session_id` 隔离;标记行不出现在输出里 —— v0.5.0;
10. **S7**:8 台实例 `concurrency=4` 的只读批量,总耗时显著低于串行且结果无串流 —— v0.5.0;
